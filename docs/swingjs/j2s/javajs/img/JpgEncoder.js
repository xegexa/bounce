...
this.dpi=300;
if (this.quality <= 0) this.quality=(params.containsKey$O("qualityJPG") ? (params.get$O("qualityJPG")).intValue$() : -1);
if (this.quality >= 90) {
...
p$1.writeDensities.apply(this, []);
p$1.writeThumbNailSize.apply(this, []);
var comment=null;
...
Clazz.newMeth(C$, 'writeDensities',  function () {
this.out.writeByteAsInt$I(1);
this.out.writeShort$H(($s$[0] = this.dpi, $s$[0]));
this.out.writeShort$H(($s$[0] = this.dpi, $s$[0]));
}, p$1);

Clazz.newMeth(C$, 'writeThumbNailSize',  function () {
this.writeArray$BA(Clazz.array(Byte.TYPE, [2]));
}, p$1);
...
C$.jfif=Clazz.array(Byte.TYPE, -1, [-1, -32, 0, 16, 74, 70, 73, 70, 0, 1, 0]);
