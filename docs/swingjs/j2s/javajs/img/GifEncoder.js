...
for (var i=this.pixels.length; --i >= 0; ) this.pixels[i]=this.pixels[i] & ~263172;
...
var c=cells.get$I(i);
colorMap.put$O$O(Integer.valueOf$I($I$(6,"colorPtToFFRGB$javajs_util_T3d",[this.palette[i]=c.setColor$()])), c);
}
...
if (pe == null  || pe.x == 1.7976931348623157E308  ) {
lab=null;
rgb=this.pixels[p];
} else {
lab=this.toLABnorm$I(this.pixels[p]);
err=pe;
err.x=p$1.clamp$D$D$D.apply(this, [err.x, -75, 75]);
err.y=p$1.clamp$D$D$D.apply(this, [err.y, -75, 75]);
err.z=p$1.clamp$D$D$D.apply(this, [err.z, -75, 75]);
lab.add$javajs_util_T3d(err);
rgb=$I$(6,"colorPtToFFRGB$javajs_util_T3d",[this.toRGB$javajs_util_P3d(lab)]);
}var key=Integer.valueOf$I(rgb);
...
if (cell == null ) {
var maxerr=1.7976931348623157E308;
for (var ib=cells.size$(); --ib >= 1; ) {
var c=cells.get$I(ib);
err.sub2$javajs_util_T3d$javajs_util_T3d(lab, c.center);
...
err.sub2$javajs_util_T3d$javajs_util_T3d(lab, cell.center);
var notLastCol=(j < this.width - 1);
if (notLastCol) p$1.addError$javajs_util_P3d$I$javajs_util_P3dA$I$I.apply(this, [err, 7, errors, p + 1, w2]);
if (notLastRow) {
if (j > 0) p$1.addError$javajs_util_P3d$I$javajs_util_P3dA$I$I.apply(this, [err, 3, errors, p + this.width - 1, w2]);
p$1.addError$javajs_util_P3d$I$javajs_util_P3dA$I$I.apply(this, [err, 5, errors, p + this.width, w2]);
if (notLastCol) p$1.addError$javajs_util_P3d$I$javajs_util_P3dA$I$I.apply(this, [err, 1, errors, p + this.width + 1 , w2]);
}}err.x=1.7976931348623157E308;
...
Clazz.newMeth(C$, 'addError$javajs_util_P3d$I$javajs_util_P3dA$I$I',  function (err, f, errors, p, w2) {
if (this.pixels[p] == this.backgroundColor) return;
p%=w2;
var errp=errors[p];
if (errp == null ) errp=errors[p]=Clazz.new_($I$(1,1));
 else if (errp.x == 1.7976931348623157E308 ) errp.set$D$D$D(0, 0, 0);
errp.scaleAdd2$D$javajs_util_T3d$javajs_util_T3d(f / 16.0, err, errp);
}, p$1);

Clazz.newMeth(C$, 'toLABnorm$I',  function (rgb) {
var lab=$I$(6).colorPtFromInt$I$javajs_util_P3d(rgb, null);
this.rgbToXyz$javajs_util_P3d$javajs_util_P3d(lab, lab);
this.xyzToLab$javajs_util_P3d$javajs_util_P3d(lab, lab);
lab.y=(lab.y + 86.185) / (184.439) * 100.0;
lab.z=(lab.z + 107.863) / (202.345) * 100.0;
return lab;
});

Clazz.newMeth(C$, 'toRGB$javajs_util_P3d',  function (lab) {
var xyz=$I$(1).newP$javajs_util_T3d(lab);
xyz.y=xyz.y / 100.0 * (184.439) - 86.185;
xyz.z=xyz.z / 100.0 * (202.345) - 107.863;
this.labToXyz$javajs_util_P3d$javajs_util_P3d(xyz, xyz);
return this.xyzToRgb$javajs_util_P3d$javajs_util_P3d(xyz, xyz);
});

Clazz.newMeth(C$, 'rgbToXyz$javajs_util_P3d$javajs_util_P3d',  function (rgb, xyz) {
if (xyz == null ) xyz=Clazz.new_($I$(1,1));
xyz.x=p$1.sxyz$D.apply(this, [rgb.x]);
xyz.y=p$1.sxyz$D.apply(this, [rgb.y]);
xyz.z=p$1.sxyz$D.apply(this, [rgb.z]);
C$.rgb2xyz.rotate$javajs_util_T3d(xyz);
return xyz;
});

Clazz.newMeth(C$, 'sxyz$D',  function (x) {
x/=255;
return (x <= 0.04045  ? x / 12.92 : Math.pow(((x + 0.055) / 1.055), 2.4)) * 100;
}, p$1);

Clazz.newMeth(C$, 'xyzToRgb$javajs_util_P3d$javajs_util_P3d',  function (xyz, rgb) {
if (rgb == null ) rgb=Clazz.new_($I$(1,1));
rgb.setT$javajs_util_T3d(xyz);
rgb.scale$D(0.01);
C$.xyz2rgb.rotate$javajs_util_T3d(rgb);
rgb.x=p$1.clamp$D$D$D.apply(this, [p$1.srgb$D.apply(this, [rgb.x]), 0, 255]);
rgb.y=p$1.clamp$D$D$D.apply(this, [p$1.srgb$D.apply(this, [rgb.y]), 0, 255]);
rgb.z=p$1.clamp$D$D$D.apply(this, [p$1.srgb$D.apply(this, [rgb.z]), 0, 255]);
return rgb;
});

Clazz.newMeth(C$, 'srgb$D',  function (x) {
return (x > 0.0031308  ? (1.055 * Math.pow(x, 0.4166666666666667)) - 0.055 : x * 12.92) * 255;
}, p$1);

Clazz.newMeth(C$, 'xyzToLab$javajs_util_P3d$javajs_util_P3d',  function (xyz, lab) {
if (lab == null ) lab=Clazz.new_($I$(1,1));
var x=p$1.flab$D.apply(this, [xyz.x / 95.0429]);
var y=p$1.flab$D.apply(this, [xyz.y / 100]);
var z=p$1.flab$D.apply(this, [xyz.z / 108.89]);
lab.x=(116 * y) - 16;
lab.y=500 * (x - y);
lab.z=200 * (y - z);
return lab;
...
Clazz.newMeth(C$, 'flab$D',  function (t) {
return (t > 0.00885645168  ? Math.pow(t, 0.333333333) : 7.78703704 * t + 0.137931034);
}, p$1);

Clazz.newMeth(C$, 'labToXyz$javajs_util_P3d$javajs_util_P3d',  function (lab, xyz) {
if (xyz == null ) xyz=Clazz.new_($I$(1,1));
xyz.setT$javajs_util_T3d(lab);
var y=(xyz.x + 16) / 116;
var x=xyz.y / 500 + y;
var z=y - xyz.z / 200;
xyz.x=p$1.fxyz$D.apply(this, [x]) * 95.0429;
xyz.y=p$1.fxyz$D.apply(this, [y]) * 100;
xyz.z=p$1.fxyz$D.apply(this, [z]) * 108.89;
return xyz;
});

Clazz.newMeth(C$, 'fxyz$D',  function (t) {
return (t > 0.206896552  ? t * t * t  : 0.128418549 * (t - 0.137931034));
}, p$1);

Clazz.newMeth(C$, 'clamp$D$D$D',  function (c, min, max) {
c=(c < min  ? min : c > max  ? max : c);
return (min == 0  ? Long.$dval(Math.round$D(c)) : c);
}, p$1);
...
{
C$.rgb2xyz=$I$(2,"newA9$DA",[Clazz.array(Double.TYPE, -1, [0.4124, 0.3576, 0.1805, 0.2126, 0.7152, 0.0722, 0.0193, 0.1192, 0.9505])]);
C$.xyz2rgb=$I$(2,"newA9$DA",[Clazz.array(Double.TYPE, -1, [3.2406, -1.5372, -0.4986, -0.9689, 1.8758, 0.0415, 0.0557, -0.204, 1.057])]);
};
...
this.setT$javajs_util_T3d(this.b$['javajs.img.GifEncoder'].toLABnorm$I.apply(this.b$['javajs.img.GifEncoder'], [rgb]));
}, 1);
...
C$.$fields$=[['D',['volume'],'I',['index'],'O',['center','javajs.util.P3d']]]
...
for (var i=count; --i >= 0; ) this.center.add$javajs_util_T3d(this.get$I(i));

this.center.scale$D(1.0 / count);
return this.b$['javajs.img.GifEncoder'].toRGB$javajs_util_P3d.apply(this.b$['javajs.img.GifEncoder'], [this.center]);
});
...
var ranges=Clazz.array(Double.TYPE, [3, 3]);
for (var ic=0; ic < 3; ic++) {
var low=1.7976931348623157E308;
var high=-1.7976931348623157E308;
for (var i=n; --i >= 0; ) {
